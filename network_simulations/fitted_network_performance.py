...
if __name__ == "__main__":
    ...
    states_L23e = StateMonitor(pop_L23e, ['V', 'epsilon', 'I_exc', 'I_inh'], record=True)
    states_L23i = StateMonitor(pop_L23i, ['V', 'epsilon', 'I_exc', 'I_inh'], record=True)
    states_L4e = StateMonitor(pop_L4e, ['V', 'epsilon', 'I_exc', 'I_inh'], record=True)
    states_L4i = StateMonitor(pop_L4i, ['V', 'epsilon', 'I_exc', 'I_inh'], record=True)
    states_L5e = StateMonitor(pop_L5e, ['V', 'epsilon', 'I_exc', 'I_inh'], record=True)
    states_L5i = StateMonitor(pop_L5i, ['V', 'epsilon', 'I_exc', 'I_inh'], record=True)
    states_L6e = StateMonitor(pop_L6e, ['V', 'epsilon', 'I_exc', 'I_inh'], record=True)
    states_L6i = StateMonitor(pop_L6i, ['V', 'epsilon', 'I_exc', 'I_inh'], record=True)
    ...
    def V_spiked(spike_monitor, state_monitor):
        V = state_monitor.V / mV
        for j in range(len(V)):
            spike_indices = spike_monitor.i
            pos = np.digitize(spike_monitor.t[spike_indices == j] / ms, state_monitor.t /ms) - 1
            V[j][pos] = 40.
        return V

    v_dict = {
        'L23e': np.mean(V_spiked(spikes_L23e, states_L23e), axis=0),
        'L23i': np.mean(V_spiked(spikes_L23i, states_L23i), axis=0),
        'L4e': np.mean(V_spiked(spikes_L4e, states_L4e), axis=0),
        'L4i': np.mean(V_spiked(spikes_L4i, states_L4i), axis=0),
        'L5e': np.mean(V_spiked(spikes_L5e, states_L5e), axis=0),
        'L5i': np.mean(V_spiked(spikes_L5i, states_L5i), axis=0),
        'L6e': np.mean(V_spiked(spikes_L6e, states_L6e), axis=0),
        'L6i': np.mean(V_spiked(spikes_L6i, states_L6i), axis=0),
    }
    ...
    I_exc_dict = {
        'L23e': states_L23e.I_exc / pA,
        'L23i': states_L23i.I_exc / pA,
        'L4e': states_L4e.I_exc / pA,
        'L4i': states_L4i.I_exc / pA,
        'L5e': states_L5e.I_exc / pA,
        'L5i': states_L5i.I_exc / pA,
        'L6e': states_L6e.I_exc / pA,
        'L6i': states_L6i.I_exc / pA,
    }

    I_inh_dict = {
        'L23e': states_L23e.I_inh / pA,
        'L23i': states_L23i.I_inh / pA,
        'L4e': states_L4e.I_inh / pA,
        'L4i': states_L4i.I_inh / pA,
        'L5e': states_L5e.I_inh / pA,
        'L5i': states_L5i.I_inh / pA,
        'L6e': states_L6e.I_inh / pA,
        'L6i': states_L6i.I_inh / pA,
    }
    ...
    if False:
        with open('network_simulations/fitted_network_performance.pkl', 'wb') as f:
